    ...
    public:
        MultigridHierarchy(int numLevels, const StaggeredGrid* finestGrid) {
            ...
            for (int i = numLevels; i >= 1; i--) {
                ...
                int kmax = finestGrid->kmax / (1 << i);
                std::cout << "Creating grid with imax = " << imax << ", jmax = " << jmax << ", and kmax = " << kmax << std::endl;
                grids.push_back(std::make_unique<StaggeredGrid>(imax, jmax, kmax, finestGrid->xlength, finestGrid->ylength, finestGrid->zlength));
            }
