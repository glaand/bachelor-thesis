check_conda_env:
	@if [ "$$CONDA_DEFAULT_ENV" != "bachelor-thesis" ]; then \
		echo "Error: The 'bachelor-thesis' environment is not active. Activate the 'bachelor-thesis' environment and try again."; \
		exit 1; \
	fi
...
all: build_folder_check configure build test
